...
int netwatch_pim_init(void **module_data, void *module_init_data)
{
	nw_pim_t     *nw;
	...
	*module_data = nw;
	...
	debug_lb(FST_DBG, "NetWatch: got ifaces = %s num=%d\n",
		 initStr, linesNum);
		
	// Setting the network interfaces names
...
int netwatch_pim_update(void *module_data)
{
	...
	if((dev_fd = open(NET_WATCH_PROC_FILE, O_RDONLY)) == -1)
		return 0;
...
int netwatch_pim_desc(void *module_data, char *buff) {
  sprintf(buff, "\t<vlen name=\"" ITEM_NET_WATCH_NAME "\" type=\"string\" unit=\"xml\" />\n" );
